...
def plot_shots(team,df_events,df_players):
    ...
    x=['xGoals Shot' , 'goals received' , ' goals scored'],
    y=[xgoals(team,df_players) , goals(team,df_events)[0] , goals(team,df_events)[1] ]
    ))
    fig.update_layout(title = 'Nombre de buts réalisés, reçus et espérés par '+team)
    return fig
################################################
def goals(team,df):
    d = df.loc[( df['Event Name'] == 'Goal')]
    received = d.loc [d['Team B'] == team]
