...
def draw_player_pass(p,m,df_events):
        ...
        return fig
#####################################################
def draw_player_shot(p, m, df_events):
    shots = ['Shot','Goal','Header Shot','Direct Free Kick Shot','Penalty Shot']
    colors = {'Goal':'darkblue', 'Shot':'purple', 'Header Shot':'yellow','Penalty Shot':'gold','Direct Free Kick Shot':'cyan'}
    d=df_events.loc[(df_events['Player1 Name']==p)&(df_events['Match Name']==m)&(df_events['Event Name'].isin(shots))]
    fig = go.Figure()
    fig = make_pitch_figure(PitchDimensions(),pitch_background=SingleColourBackground("#81B623"))
    ## Add Z variable for xG
    z = d ['xG Score'].tolist()
    data=[i for i in d['Event Name'].values if i in colors.keys()]
    z1 = [150 * i+10 for i in z] # This is to scale the "xG" values for plotting
    ...
    orientation="h"),title='Les tires de '+p)
    return fig
#################################################
